...
mu_kappi <- mean(training_set$rating)
#RMSE of Model 1
RMSE_MODEL_1 <- RMSE_Formula(b, mu_kappi)
RMSE_MODEL_1
...
complaints_rating <- training_set %>%
  group_by(complaints) %>%
  summarise(x = mean(rating - mu_kappi))
complaints_rating_forecast <- mu_kappi + testing_set %>%
...
#RMSE of Model 4
RMSE_MODEL_3 <- RMSE_Formula(b, sd_cap)
RMSE_MODEL_3

#INSIGHTS GAINED FROM MODEL 3:
#On the bright side, this model managed to produce a RMSE reading with digits and not 'N/A'. However, this RMSE reading is not lower than the first model. This means that I will have to try another method in order to better my RMSE result.
...
set.seed(1)
matrif_r <- Reco()
matriftest <- with(testing_set, data_memory(rating = rating, user_index = complaints,
                                            item_index = raises))
matriftrain <- with(training_set, data_memory(rating = rating, user_index = complaints,
                                              item_index = raises))
matrif_p <- matrif_r$tune(matriftrain, opts = list(niter = 10, nthread = 1, dim = c(1, 20)))
matrif_r$train(matriftrain, opts = c(matrif_p$min, niter = 10, nthread = 1))

matrif_ans <- matrif_r$predict(matriftest, out_memory())
#RMSE OF MODEL 5
RMSE_MODEL_4 <- RMSE_Formula(b, matrif_ans)
RMSE_MODEL_4

#INSIGHTS GAINED FROM MODEL 4:
#This is great! I have finally succeeded in obtaining an RMSE value which is lower than all previous results. I am extremely pleased with this result. I would've liked the RMSE to be lower than this, but I'm assuming it may have something to do with the data set that I decided to choose.
